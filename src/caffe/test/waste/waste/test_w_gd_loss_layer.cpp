...
class WGdLossLayerTest : public ::testing::Test
{
  typedef TypeParam Dtype;
 protected:
  WGdLossLayerTest() : blob_bottom_(new Blob<Dtype>()), blob_top_(new Blob<Dtype>())
  {
...
TYPED_TEST_CASE(WGdLossLayerTest, myTypes);


TYPED_TEST(WGdLossLayerTest, TestGradient)
{
  typedef TypeParam Dtype;

  LayerParameter layer_param;
	Caffe::set_gan_type(TRAINDNET);
	layer_param.mutable_loss_param()->set_single_term(true);
  WGdLossLayer<Dtype> layer(layer_param);
