...
#include "caffe/layers/styleloss_layer.hpp"
...
class StyleLossLayerTest : public ::testing::Test
{
  typedef TypeParam Dtype;
 protected:
  StyleLossLayerTest() : blob_bottom_0_(new Blob<Dtype>()),blob_bottom_1_(new Blob<Dtype>()), blob_top_(new Blob<Dtype>())
  {
...
TYPED_TEST_CASE(StyleLossLayerTest, myTypes);


TYPED_TEST(StyleLossLayerTest, TestGradient)
{
  typedef TypeParam Dtype;

  LayerParameter layer_param;
  StyleLossLayer<Dtype> layer(layer_param);
  GradientChecker<Dtype> checker(1e-4, 1e-4);
