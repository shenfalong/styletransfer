...
class RepmatLayerTest : public ::testing::Test
{
  typedef TypeParam Dtype;
 protected:
  RepmatLayerTest() : blob_bottom_(new Blob<Dtype>()),blob_bottom_1_(new Blob<Dtype>()), blob_top_(new Blob<Dtype>())
  {
...
TYPED_TEST_CASE(RepmatLayerTest, myTypes);


TYPED_TEST(RepmatLayerTest, TestGradient)
{
  typedef TypeParam Dtype;

  LayerParameter layer_param;
  RepmatLayer<Dtype> layer(layer_param);
