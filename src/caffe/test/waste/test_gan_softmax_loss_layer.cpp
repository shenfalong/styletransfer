...
class GANSoftmaxWithLossLayerTest : public ::testing::Test
{
  typedef TypeParam Dtype;
 protected:
  GANSoftmaxWithLossLayerTest() : blob_bottom_0_(new Blob<Dtype>()),blob_bottom_1_(new Blob<Dtype>()), blob_top_(new Blob<Dtype>())
  {
  ...
  virtual void SetUp()
  {
    blob_bottom_0_->Reshape(4, 3, 6, 5);
		blob_bottom_1_->Reshape(2, 1, 6, 5);
    ...
    caffe_rng_gaussian<Dtype>(this->blob_bottom_0_->count(), 
   	...
   	for (int i=0;i<this->blob_bottom_1_->count();i++)
   		this->blob_bottom_1_->mutable_cpu_data()[i] = caffe_rng_rand() % 3;													
 												
    blob_bottom_vec_.push_back(blob_bottom_0_);
    blob_bottom_vec_.push_back(blob_bottom_1_);
    blob_top_vec_.push_back(blob_top_);
  }
  virtual ~GANSoftmaxWithLossLayerTest()
  {
...
TYPED_TEST_CASE(GANSoftmaxWithLossLayerTest, myTypes);


TYPED_TEST(GANSoftmaxWithLossLayerTest, TestGradient)
{
  typedef TypeParam Dtype;
  ...
  GANSoftmaxWithLossLayer<Dtype> layer(layer_param);
