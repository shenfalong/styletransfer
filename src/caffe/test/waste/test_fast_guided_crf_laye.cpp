...
protected:
  ...
  virtual void SetUp()
  {
    blob_bottom_->Reshape(2, 2, 4, 4);
    blob_bottom_1_->Reshape(2, 3, 4, 4);
    ...
    //  blob_bottom_->mutable_cpu_data()[i]=Dtype(0);
...
TYPED_TEST(TemplateLayerTest, TestGradient)
{
  ...
  layer_param.mutable_crf_param()->set_max_iter(2);
  layer_param.mutable_crf_param()->set_radius(Dtype(1));
  layer_param.mutable_crf_param()->set_eps(Dtype(1));
  layer_param.mutable_crf_param()->set_alpha(Dtype(-1));
  layer_param.add_param()->set_lr_mult(Dtype(0));
  
  FastGuidedCRFLayer<Dtype> layer(layer_param);
