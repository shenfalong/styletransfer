...
#include "caffe/layers/operator/param_cudnn_conv_layer.hpp"
...
class ParamCuDNNConvolutionLayerTest : public ::testing::Test
{
  typedef TypeParam Dtype;
 protected:
  ParamCuDNNConvolutionLayerTest() : blob_bottom_0_(new Blob<Dtype>()),blob_bottom_1_(new Blob<Dtype>()), blob_top_(new Blob<Dtype>())
  {
  ...
  virtual ~ParamCuDNNConvolutionLayerTest()
  {
...
TYPED_TEST_CASE(ParamCuDNNConvolutionLayerTest, myTypes);


TYPED_TEST(ParamCuDNNConvolutionLayerTest, TestGradient)
{
  typedef TypeParam Dtype;
  ...
  ParamCuDNNConvolutionLayer<Dtype> layer(layer_param);
