...
SolverAuxSecGAN::SolverAuxSecGAN(const SolverParameter& param)
{
  //---------------- initial train net -----------------------
  param_ = param;
  ...
  d_net_->set_NetOptimizer(param_.d_net_opt());
	...
	d_interp_net_.reset(new Net(d_interp_net_param, net_intput_blobs, net_intput_blob_names));
  d_interp_net_->set_NetOptimizer(param_.d_net_opt());
	this->share_weight(d_net_, d_interp_net_);
	//------------------------------------------------------
	
	//------------------- Auxiliary Classifier---------------------------
	NetParameter d_aux_net_param;
	ReadProtoFromTextFile(param_.d_aux_net(), &d_aux_net_param);
	d_aux_net_.reset(new Net(d_aux_net_param, net_intput_blobs, net_intput_blob_names));
  d_aux_net_->set_NetOptimizer(param_.d_net_opt());
	//------------------------------------------------------
...
void SolverAuxSecGAN::Solve(const char* all_state_file) 
{
...
//----------------------------------------------- 	
		...
		d_aux_net_->BcastData();
		d_aux_net_->Forward();
		d_aux_net_->Backward();
		//Caffe::set_bn_state("learned");
		Caffe::set_frozen_param(false);
		//-----------------------------------------------	
...
void SolverAuxSecGAN::dispaly_loss(std::vector<float> g_loss,std::vector<float> d_loss) 
{
...
void SolverAuxSecGAN::Restore(const char* all_state_file) 
{
	SolverState all_state;
